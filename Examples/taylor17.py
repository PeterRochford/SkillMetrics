...
A seventeenth example of how to create a Taylor diagram given one set of
reference observations and two sets of model predictions for the quantity. 
This is a variant of example 13, but where a different pair of marker & 
color are used for each set of data points. 
...
if __name__ == '__main__':
    ...
    # for each data set. Assign unique pair of color + marker for each label.
    era5 = {"color" : "r", "marker" : "*"}
    trmm = {"color" : "b", "marker" : "."}
    label = {'ERA-5': era5, 'TRMM': trmm}
    ...
    sm.taylor_diagram(stats.taylor_stats1['sdev'], 
                      ...
                      stats.taylor_stats1['ccoef'], markercolor = str(era5["color"]), alpha = 0.0,
                      markersymbol = str(era5["marker"]), 
    ...
    sm.taylor_diagram(stats.taylor_stats2['sdev'], 
                      ...
                      stats.taylor_stats2['ccoef'], markercolor = str(trmm["color"]), alpha = 0.0,
                      markersymbol = str(trmm["marker"]), 
                      overlay = 'on', markerLabel = label)

    # Write plot to file if arguments say so
    None if args.no_save else plt.savefig('taylor17.png', dpi=150, facecolor='w')
